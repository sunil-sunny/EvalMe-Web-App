...
public enum SQLQueries {
    ...
    GET_USER_ROLES("select s2.bannerid,rolename from role inner join (select s.roleid,s.bannerid from (select roleid,bannerid from courserole union select roleid,bannerid from systemrole) as s where bannerid = ? ) as s2 on s2.roleid = role.roleid"),
